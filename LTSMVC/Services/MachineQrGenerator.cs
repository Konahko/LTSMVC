...
{
    public class MachineQrGenerator
    {
        ...
        public MemoryStream GetQrImageStream(Machine item, QrCodeSize size)
        {
            ...
            AddTextToImage(g, size, item.Id, item.Name, item.Character, dateTime);
        ...
        private void AddTextToImage(Graphics g, QrCodeSize size, int itemId, string itemName, string itemCharacter, DateTime dateTime)
        {
            switch (size)
            {
                ...
                case QrCodeSize.Medium:
                {
                    ...
                    g.DrawString(itemCharacter, font, Brushes.Black, 350, 162);
